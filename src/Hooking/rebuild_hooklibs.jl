...
function rebuild_lib(file, triple, oname)
    ...
    for (pat, rep) in replacements
        println(input, ".set $pat,$rep")
    end
    basic_size = length(basic_regs)*sizeof(Ptr{Void})
    println(input, ".set UC_MCONTEXT_SIZE,$basic_size")
    total_size = basic_size
    if Sys.ARCH == :x86_64
        xsave_area_size = sizeof(fieldtype(Gallium.X86_64.ExtendedRegs,:xsave_state))
        total_size += xsave_area_size
    end
    println(input, ".set UC_MCONTEXT_TOTAL_SIZE,$total_size")
    println(input, ".include \"$file\"")
    cmd = `$(joinpath(JULIA_HOME,"../tools/llvm-mc")) -triple=$triple -filetype=obj - -o $oname`
    (stdout,stdin,process) = readandwrite(pipeline(cmd,stderr=STDERR))
    write(stdin, takebuf_array(input))
    close(stdin)
    wait(process)
...
if Sys.ARCH == :x86_64
    ...
    if is_windows() || haskey(ENV, "WIN64_SYSROOT")
        win64_sysroot = haskey(ENV, "WIN64_SYSROOT") ? `--sysroot=$(ENV["WIN64_SYSROOT"])` : ``
        ...
        run(`clang -target $windows_triple $win64_sysroot -fPIC -c -o coff-callback-x86_64.o callback.c`)
        run(`clang -target $windows_triple $win64_sysroot -shared -o hooking-x86_64.dll coffjump-x86_64.o coffhook-x86_64.o coff-callback-x86_64.o`)
    end
else
